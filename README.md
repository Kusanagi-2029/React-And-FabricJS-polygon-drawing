...
- [Описание проекта](#описание-проекта)
    - [Видео/Gif с DEMO](#Видео/Gif-с-DEMO)
...
> [!IMPORTANT]  
> **Адаптивность** и **отзывчивость** - интерфейс весьма отзывчив на разных разрешениях, ориентациях экрана и устройствах. Продемонстрировано на Demo-видео.
![image](https://github.com/user-attachments/assets/83dac997-41cc-4aba-adfd-210cdbe0f873)
...
Запускает code formatting по всему проекту - актуально для пользователей VS Code (WebStorm корректнее работает с конфигами линтеров и prettier'a).
...
1. **Функциональная модульность**: Код разбивается на модули, где каждый модуль представляет собой независимую функциональность. Это позволяет легче поддерживать и расширять функционал приложения.
   - **Пример** - данный модуль отрисовывает canvas-холст и отвечает за связанную с ним логику.
...
3. **Масштабируемость**: Упрощает добавление нового функционала, так как новые функции добавляются в отдельные модули, не затрагивая существующие.
   - **Пример** - Вся директория **"helpers"** - она может быть нужна только данному модулю, следовательно, находится в нём и не экспортируется во внешнее приложение.
...
4. **Упрощенное тестирование**: Модули можно тестировать отдельно, что делает процесс тестирования более управляемым и эффективным.
   - **Пример** - тест только компонента из данного модуля **`ControlPanel.test.tsx`**.
